...
# ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ 
# define the variables pertaining to your own target wikibase instance.
# ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ 

wikibase_name="WikibaseFS"

wikibase_address="fluidseal.smarter.codes"

query_address="query-fluidseal.smarter.codes"

reconcile_address="http://reconciliation-fluidseal.smarter.codes"
...
echo "
{
    ...
    \"mediawiki\": {
      \"name\": \"${wikibase_name}\",
      \"root\": \"http://${wikibase_address}/wiki/\",
      \"main_page\": \"http://${wikibase_address}/wiki/Main_Page\",
      \"api\": \"http://${wikibase_address}/w/api.php\"
    },
    \"wikibase\": {
      \"site_iri\": \"http://${wikibase_address}/entity/\",
      \"maxlag\": 5,
    ...
    \"entity_types\": {
      \"item\": {
         ...
         \"mediawiki_api\": \"http://${wikibase_address}/w/api.php\"
      },
      \"property\": {
         \"site_iri\": \"http://${wikibase_address}/entity/\",
         \"mediawiki_api\": \"http://${wikibase_address}/w/api.php\"
...
mediawiki_api_endpoint = 'http://${wikibase_address}/w/api.php'

# SPARQL endpoint
wikibase_sparql_endpoint = 'http://${query_address}/proxy/wdqs/bigdata/namespace/wdq/sparql'

# Name of the Wikibase instance
wikibase_name = '${wikibase_name}'

# URL of the main page of the Wikibase instance
wikibase_main_page = 'http://${wikibase_address}/wiki/Main_Page'
...
import re
q_re = re.compile(r'(<?http?://${wikibase_address}/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?http?://${wikibase_address}/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3

# Identifier space and schema space exposed to OpenRefine.
# This should match the IRI prefixes used in RDF serialization.
# Note that you should be careful about using http or http there,
# because any variation will break comparisons at various places.
identifier_space = 'http://${wikibase_address}/entity/'
schema_space = 'http://${wikibase_address}/prop/direct/'

# Pattern used to form the URL of a Qid.
# This is only used for viewing so it is fine to use any protocol (therefore, preferably http if supported)
qid_url_pattern = 'http://${wikibase_address}/wiki/Item:{{id}}'
...
image_download_pattern = 'http://upload.wikimedia.org/wikipedia/commons/thumb/%s/%s/%s/%dpx-%s'
...
autodescribe_endpoint = 'http://tools.wmflabs.org/autodesc/'
